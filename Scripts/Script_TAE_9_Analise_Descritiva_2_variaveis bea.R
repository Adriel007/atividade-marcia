...
# Criar um gradiente de cores entre azul e verde
gradiente_cores <- colorRampPalette(c("#00EEEE", "#00CDCD", "cyan4"))(n = length(tabela_contingencia))
par(mfrow = c(1, 2))
barplot(tabela_contingencia, 
        main = "Estado Civil", 
        xlab = "Nível de Instrução", 
        ylab = "Frequência", 
        beside = TRUE,
        col = gradiente_cores,  # Aplique o gradiente de cores
        legend = TRUE)
barplot(t(tabela_contingencia),
        main = "Nível de Instrução", 
        xlab = "Estado Civil", 
        ylab = "Frequência", 
        beside = TRUE,
        col = gradiente_cores,  # Aplique o gradiente de cores       
...
# Calcular o coeficiente de contingência a partir da fórmula executando o teste Qui-quadrado (WARNING)
tabela <- table(milsa$Est.civil, milsa$Inst)
qui_quadrado <- chisq.test(tabela)

# correção de Yates (bea)
qui_quadrado <- chisq.test(tabela, correct = TRUE)
qui_quadrado

# Aplicando o teste exato de Fisher (bea)
fisher_result <- fisher.test(tabela)
fisher_result
