...
----------------------------------------------------------------------------------------------------------------------
# Histograma do Salário com várias linhas
hist_salario <- hist(ceagfgv$salario, main = "Histograma de Salários", 
                     xlab = "Salário (em reais)", ylab = "Frequência", 
                     col = "skyblue", border = "black", breaks = 15, freq = FALSE)

# Adicionando a linha de densidade
lines(density(ceagfgv$salario), col = "darkgreen", lwd = 2)

# Adicionando a linha da média
abline(v = mean(ceagfgv$salario), col = "red", lwd = 2, lty = 2)

# Adicionando a linha da mediana
abline(v = median(ceagfgv$salario), col = "blue", lwd = 2, lty = 2)

# Adicionando uma linha horizontal (opcional)
abline(h = 0.01, col = "purple", lwd = 2)  # linha horizontal em um valor de densidade

# Histograma dos Anos Formado com várias linhas
hist_anosformado <- hist(ceagfgv$anosformado, main = "Histograma de Anos de Formação", 
                         xlab = "Anos de Formação", ylab = "Frequência", 
                         col = "lightgreen", border = "black", breaks = 10, freq = FALSE)

# Adicionando a linha de densidade
lines(density(ceagfgv$anosformado), col = "darkorange", lwd = 2)

# Adicionando a linha da média
abline(v = mean(ceagfgv$anosformado), col = "red", lwd = 2, lty = 2)  # linha vermelha pontilhada para a média

# Adicionando a linha da mediana
abline(v = median(ceagfgv$anosformado), col = "blue", lwd = 2, lty = 2)  # linha azul pontilhada para a mediana

# Adicionando uma linha horizontal (opcional) em um valor de densidade específico
abline(h = 0.05, col = "purple", lwd = 2)  # linha horizontal em um valor de densidade